...
std::string to_lower(std::string_view value) noexcept {
    ...
    std::transform(value.begin(), value.end(), result.begin(),
                   [](char symbol) { return static_cast<unsigned char>(std::tolower(symbol)); });
...
bool equal_case_insensitive_functor(const std::string_view &left,
                                    const std::string_view &right) noexcept {
    return left.size() == right.size() &&
           std::equal(left.cbegin(), left.cend(), right.cbegin(), right.cend(), equal_char_ascii);
}

bool equal_case_insensitive_lambda(const std::string_view &left,
                                   const std::string_view &right) noexcept {
    return (left.size() == right.size()) &&
...
static void case_insensitive_equal_functor(benchmark::State &state, const std::string &lhs,
...
static void case_insensitive_equal_lambda(benchmark::State &state, const std::string &lhs,
                                          const std::string &rhs) {
    for (auto _ : state) {
...
static void case_insensitive_less_simple(benchmark::State &state, const std::string &lhs,
                                         const std::string &rhs) {
    for (auto _ : state) {
...
static void case_insensitive_less_functor(benchmark::State &state, const std::string &lhs,
                                          const std::string &rhs) {
    for (auto _ : state) {
...
static void case_insensitive_less_lambda(benchmark::State &state, const std::string &lhs,
                                         const std::string &rhs) {
    for (auto _ : state) {
...
static void case_insensitive_less_compare(benchmark::State &state, const std::string &lhs,
                                          const std::string &rhs) {
