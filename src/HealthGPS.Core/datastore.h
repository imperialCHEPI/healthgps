...
namespace hgps {
	namespace core {
		...
		{
		public:
			/// @brief Destroys a hgps::core::Datastore instance
			...
			virtual std::vector<PopulationItem> get_population(
				Country country, const std::function<bool(const unsigned int&)> time_filter) const = 0;
			...
			virtual RelativeRiskEntity get_relative_risk_to_risk_factor(
				DiseaseInfo source, Gender gender, Identifier risk_factor_key) const = 0;
			...
			virtual std::vector<BirthItem> get_birth_indicators(
				const Country country, const std::function<bool(const unsigned int&)> time_filter) const = 0;
