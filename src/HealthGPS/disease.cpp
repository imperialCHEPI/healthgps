...
void reset_disease_caches() {
    try {
        ...
        std::cerr << "WARNING: Error during disease cache reset, creating new cache instances" << std::endl;
...
void DiseaseModule::update_population(RuntimeContext &context) {
    ...
    for (const auto &disease_id : disease_ids) {
        ...
        try {
            ...
            if (!models_.contains(disease_id) || !models_.at(disease_id)) {
                ...
                if (disease_update_failures[disease_name]) {
                    std::cerr << "WARNING: Disease " << disease_name 
                              << " has failed multiple times and will be skipped in future runs" << std::endl;
                } else {
            ...
            std::cout << "DEBUG: [DiseaseModule] Successfully updated " 
                      << disease_name << " disease status" << std::endl;
    ...
    std::cout << "INFO: [DiseaseModule] Updated " << success_count << " disease models successfully, "
              << error_count << " failures" << std::endl;
    ...
    std::cout << "INFO: Initialized " << name() << " for " 
              << success_count << " out of " << context.population().current_active_size() << " active people (" 
              << (success_count * 100.0 / std::max(1, static_cast<int>(context.population().current_active_size()))) << "%), with " 
...
bool DiseaseModule::has_disease(const core::Identifier &disease_code, const Person &entity) const noexcept {
    try {
...
double DiseaseModule::get_excess_mortality(const core::Identifier &disease_code,
    ...
    try {
        ...
        std::cerr << "ERROR: Map key error in get_excess_mortality for disease " 
                  << disease_name << ": " << e.what() << std::endl;
        ...
        std::cerr << "ERROR: Failed to get excess mortality for disease " 
                  << disease_name << ": " << e.what() << std::endl;
        ...
        std::cerr << "ERROR: Unknown exception in get_excess_mortality for disease "
                  << disease_name << std::endl;
...
std::unique_ptr<DiseaseModule> build_disease_module(Repository &repository,
    ...
    tbb::parallel_for_each(std::begin(validated_diseases), 
                          std::end(validated_diseases), [&](auto &info) {
        auto info_code_str = info.code.to_string();
        try {
            ...
            // Keep critical error
            std::cerr << "ERROR: Failed to initialize disease model for " 
                      << info_code_str << ": " << e.what() << std::endl;
            failed_count++;
    ...
    std::cout << "INFO: Disease initialization complete: " << successful_count 
              << " successful, " << failed_count << " failed" << std::endl;
