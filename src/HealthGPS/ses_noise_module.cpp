    ...
    : function_{function}, parameters_{parameters} 
{
    if (!core::case_insensitive::equals("normal", function)) 
        throw std::invalid_argument(fmt::format("Noise generation function: {} is not supported", function));

    if (parameters.size() != 2) 
        throw std::invalid_argument(fmt::format("Number of parameters mismatch: expected 2, received {}", parameters.size()));
}
...
void SESNoiseModule::update_population(RuntimeContext &context)
{
    ...
    auto indices = core::find_index_of_all(pop, [&](const Person &entity) { return entity.age == newborn_age; });
