...
RiskFactorInspector::RiskFactorInspector(const std::filesystem::path &output_dir)
    ...
    auto baseline_path = output_dir / ("Year3_Baseline_Individual_RiskFactors_" + timestamp_str + ".csv");
    auto intervention_path = output_dir / ("Year3_Intervention_Individual_RiskFactors_" + timestamp_str + ".csv");
    ...
    if (!baseline_file_.is_open()) {
        throw core::HgpsException("MAHIMA: Failed to create baseline inspection file: " + baseline_path.string());
    }
    
    if (!intervention_file_.is_open()) {
        throw core::HgpsException("MAHIMA: Failed to create intervention inspection file: " + intervention_path.string());
    }
...
void RiskFactorInspector::capture_year_3_data(RuntimeContext &context) {
    ...
    std::cout << "\n  Population to process: " << active_count << " active out of " << total_count << " total";
    ...
    for (const auto &person : context.population()) {
        ...
        write_person_record(*target_file, person, scenario_type, 
                           context.current_run(), context.time_now());
        records_written++;
    }
