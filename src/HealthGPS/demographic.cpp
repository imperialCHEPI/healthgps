...
DemographicModule::DemographicModule(
    ...
    , expected_{std::make_shared<UnorderedMap2d<core::Gender, core::Identifier, std::map<int, double>>>()}
...
void DemographicModule::initialise_age_gender(RuntimeContext &context, Population &population,[[maybe_unused]] Random &random) {
    ...
    if (age_gender_dist.empty() || (age_gender_dist.size() == 1 && age_gender_dist.begin()->first == 100) || 
        ...
        for (int age = 0; age <= 100; age++) {
            ...
            if (population.size() < 100 && age % 10 == 0) {
                weight = std::max(weight, 0.05); // Force minimum weight for decade ages in small populations
            }
            
            total_weight += weight;
            age_gender_dist[age] = DoubleGenderValue(weight/2.0, weight/2.0); // Equal gender distribution
        }
    ...
    if (entry_total > 0) {
        ...
        for (const auto& [age, value] : age_gender_dist) {
            if (count < 5) {
                std::cout << "DEBUG: Age " << age << " - Males: " << value.males << ", Females: " << value.females << std::endl;
                count++;
            } else {
                break;
            }
    ...
    if (pop_size < 100) {
        ...
        int reserved_count = required_ages.size() * 2; // One male and one female for each required age
        ...
        if (index < pop_size) {
            ...
            for (auto entry_count = 1; auto &entry : age_gender_dist) {
                ...
                if (entry.second.males > 0 && num_males == 0) num_males = 1;
                if (entry.second.females > 0 && num_females == 0) num_females = 1;
    ...
    std::cout << "  Age 0-20: " << std::accumulate(age_count.begin(), 
                                std::upper_bound(age_count.begin(), age_count.end(), 20, 
                                                [](int v, const auto& p){ return v < p.first; }),
                                0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 21-40: " << std::accumulate(std::lower_bound(age_count.begin(), age_count.end(), 21, 
                                                  [](const auto& p, int v){ return p.first < v; }), 
                                                std::upper_bound(age_count.begin(), age_count.end(), 40, 
                                                                  [](int v, const auto& p){ return v < p.first; }),
                                                0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 41-60: " << std::accumulate(std::lower_bound(age_count.begin(), age_count.end(), 41, 
                                                  [](const auto& p, int v){ return p.first < v; }), 
                                                std::upper_bound(age_count.begin(), age_count.end(), 60, 
                                                                  [](int v, const auto& p){ return v < p.first; }),
                                                0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 61-80: " << std::accumulate(std::lower_bound(age_count.begin(), age_count.end(), 61, 
                                                  [](const auto& p, int v){ return p.first < v; }), 
                                                std::upper_bound(age_count.begin(), age_count.end(), 80, 
                                                                  [](int v, const auto& p){ return v < p.first; }),
                                                0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 81+: " << std::accumulate(std::lower_bound(age_count.begin(), age_count.end(), 81, 
                                                [](const auto& p, int v){ return p.first < v; }), 
                                              age_count.end(),
                                              0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
}
...
// This initializes the enrire virtual population with age, gender, region (set to 1), ethnicity, income_continuous, income_category and physical activity
void DemographicModule::initialise_population(RuntimeContext &context, Population &population, Random &random) {
    ...
    std::cout << "  Age 0-20: " << std::accumulate(age_groups.begin(), 
                                                 std::upper_bound(age_groups.begin(), age_groups.end(), 20, 
                                                                  [](int v, const auto& p){ return v < p.first; }),
                                                 0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 21-40: " << std::accumulate(std::lower_bound(age_groups.begin(), age_groups.end(), 21, 
                                                                   [](const auto& p, int v){ return p.first < v; }), 
                                                  std::upper_bound(age_groups.begin(), age_groups.end(), 40, 
                                                                   [](int v, const auto& p){ return v < p.first; }),
                                                  0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 41-60: " << std::accumulate(std::lower_bound(age_groups.begin(), age_groups.end(), 41, 
                                                                   [](const auto& p, int v){ return p.first < v; }), 
                                                  std::upper_bound(age_groups.begin(), age_groups.end(), 60, 
                                                                   [](int v, const auto& p){ return v < p.first; }),
                                                  0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 61-80: " << std::accumulate(std::lower_bound(age_groups.begin(), age_groups.end(), 61, 
                                                                   [](const auto& p, int v){ return p.first < v; }), 
                                                  std::upper_bound(age_groups.begin(), age_groups.end(), 80, 
                                                                   [](int v, const auto& p){ return v < p.first; }),
                                                  0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    std::cout << "  Age 81+: " << std::accumulate(std::lower_bound(age_groups.begin(), age_groups.end(), 81, 
                                                                 [](const auto& p, int v){ return p.first < v; }), 
                                                age_groups.end(),
                                                0, [](int sum, const auto& p){ return sum + p.second; }) << " people" << std::endl;
    ...
    for (auto &person : population) {
        if (person.is_active()) {
            ...
            switch (person.income_category) {
                case core::Income::low: low_count++; break;
                case core::Income::lowermiddle: lower_middle_count++; break;
                case core::Income::uppermiddle: upper_middle_count++; break;
                case core::Income::high: high_count++; break;
            }
        }
    }
    ...
    for (const auto &person : population) {
        if (person.is_active() && examples_shown < 10) {
            std::string category;
            switch (person.income_category) {
                case core::Income::low: category = "Low"; break;
                case core::Income::lowermiddle: category = "Lower Middle"; break;
                case core::Income::uppermiddle: category = "Upper Middle"; break;
                case core::Income::high: category = "High"; break;
                default: category = "Unknown";
            }
            
            std::cout << "  Person " << examples_shown << " - Age: " << person.age 
                      << ", Income: " << person.income_continuous 
...
void DemographicModule::update_population(RuntimeContext &context,
    ...
    try {
        ...
        if (residual_future.wait_for(std::chrono::seconds(60)) == std::future_status::ready) {
            ...
            try {
                ...
                std::cout << "DEBUG: Residual mortality calculation completed successfully" << std::endl;
            } 
            catch (const std::exception& e) {
                std::cerr << "ERROR: Exception during residual mortality calculation: " << e.what() << std::endl;
            }
            catch (...) {
                std::cerr << "ERROR: Unknown exception during residual mortality calculation" << std::endl;
            }
        } else {
            std::cerr << "ERROR: Residual mortality calculation timed out after 60 seconds" << std::endl;
            // Let the future continue in background, but we'll proceed without waiting for it
        }
    }
    catch (const std::exception& e) {
        std::cerr << "ERROR: Exception while waiting for residual mortality: " << e.what() << std::endl;
    }
    catch (...) {
    ...
    if (!residual_success) {
        std::cout << "WARNING: Using default residual mortality values due to calculation failure" << std::endl;
        ...
        for (int age = 0; age <= 110; age++) {
            residual_death_rates_.at(age, core::Gender::male) = 0.001 + (age / 1000.0);   // Small age-dependent rate
            residual_death_rates_.at(age, core::Gender::female) = 0.001 + (age / 1200.0); // Slightly lower for females
        }
    }
...
void DemographicModule::update_residual_mortality(RuntimeContext &context,
    ...
    try {
        ...
        for (const auto& entity : pop) {
            if (entity.is_active()) {
                active_count++;
                if (active_count >= 10) break; // Stop counting after finding enough people
            }
        }
        
        if (active_count < 10) {
            std::cerr << "WARNING: Very small active population (" << active_count << ") for residual mortality calculation" << std::endl;
        ...
        tbb::parallel_for_each(pop.cbegin(), pop.cend(), [&](const auto &entity) {
            try {
                ...
                if (entity.age < 0 || entity.age > 110) {
                    std::string err = "Invalid age " + std::to_string(entity.age) + " for person " + std::to_string(entity.id());
                ...
                try {
                    ...
                    auto lock = std::unique_lock{sum_mutex};
                    errors.push_back(std::string("Error calculating excess mortality: ") + e.what());
        ...
        if (!errors.empty()) {
            std::cerr << "WARNING: " << errors.size() << " errors during residual mortality calculation" << std::endl;
        ...
        for (int age = start_age; age <= end_age; age++) {
            if (residual_mortality.contains(age)) {
                residual_death_rates_.at(age, core::Gender::male) = residual_mortality.at(age, core::Gender::male);
                residual_death_rates_.at(age, core::Gender::female) = residual_mortality.at(age, core::Gender::female);
            }
        }
        
        std::cout << "DEBUG: [update_residual_mortality] Calculation completed successfully" << std::endl;
    } catch (const std::exception& e) {
...
int DemographicModule::update_age_and_death_events(RuntimeContext &context,
    ...
    std::cout << "  Age 0-20: " << std::accumulate(age_counts.begin(), age_counts.upper_bound(20), 0,
        [](int sum, const auto& pair) { return sum + pair.second; }) << " people" << std::endl;
    std::cout << "  Age 21-40: " << std::accumulate(age_counts.lower_bound(21), age_counts.upper_bound(40), 0,
        [](int sum, const auto& pair) { return sum + pair.second; }) << " people" << std::endl;
    std::cout << "  Age 41-60: " << std::accumulate(age_counts.lower_bound(41), age_counts.upper_bound(60), 0,
        [](int sum, const auto& pair) { return sum + pair.second; }) << " people" << std::endl;
    std::cout << "  Age 61-80: " << std::accumulate(age_counts.lower_bound(61), age_counts.upper_bound(80), 0,
        [](int sum, const auto& pair) { return sum + pair.second; }) << " people" << std::endl;
    std::cout << "  Age 81+: " << std::accumulate(age_counts.lower_bound(81), age_counts.end(), 0,
        [](int sum, const auto& pair) { return sum + pair.second; }) << " people" << std::endl;
    ...
    for (auto &entity : context.population()) {
        ...
        if (entity.age >= max_age) {
            ...
            if (samples_shown < 3) {
                std::cout << "  Person died (max age): Age " << entity.age 
                          << ", Gender: " << (entity.gender == core::Gender::male ? "Male" : "Female") 
                          << std::endl;
                samples_shown++;
            }
        } else {
            ...
            if (samples_shown < 5) {
                std::cout << "  Person check: Age " << entity.age 
                          << ", Gender: " << (entity.gender == core::Gender::male ? "Male" : "Female") 
                          << ", Death prob: " << death_probability
                          << ", Hazard: " << hazard
                          << ", Residual rate: " << residual_death_rate
                          << std::endl;
                samples_shown++;
            }
    ...
    std::cout << "DEBUG: Deaths summary - Total: " << number_of_deaths 
              << ", Age-related: " << age_deaths 
              << ", Probability-based: " << probability_deaths 
...
void DemographicModule::initialise_ethnicity(RuntimeContext &context, Person &person, Random &random) {
    ...
    try {
        ...
        auto prevalence_data = context.inputs().get_ethnicity_probabilities(person.age, person.gender, person.region);
...
void DemographicModule::initialise_income_continuous(Person &person, Random &random) {
    ...
    try {
        ...
        if (!income_models_.empty() && income_models_.begin()->second.coefficients.count("Age") > 0) {
        ...
        if (!income_models_.empty() && income_models_.begin()->second.coefficients.count("Gender") > 0) {
            income_base += income_models_.begin()->second.coefficients.at("Gender") * gender_value;
        }

        // Add region - apply coefficient to region value, safely checking if the region exists in the map
        ...
        // Add ethnicity - apply coefficient to ethnicity value, safely checking if the ethnicity exists in the map
...
void DemographicModule::initialise_income_category(Person &person, double q1_threshold, double q2_threshold, double q3_threshold) {
    ...
    if (debug_income) {
        std::cout << "Income thresholds: Q1=" << q1_threshold << ", Q2=" << q2_threshold 
                  << ", Q3=" << q3_threshold << ", Person income=" << person.income_continuous << std::endl;
    }
    
    // Assign income categories based on quartiles - using < instead of <= for proper quartile division
...
std::tuple<double, double, double> DemographicModule::calculate_income_thresholds(const Population &population) {
...
void DemographicModule::initialise_physical_activity(RuntimeContext &context, Person &person, Random &random) {
    try {
...
double DemographicModule::get_expected([[maybe_unused]] RuntimeContext &context, core::Gender gender, int age,
    ...
    if (!expected_ || !expected_->contains(gender, factor) || !expected_->at(gender, factor).contains(age)) {
...
std::unique_ptr<DemographicModule> build_population_module(Repository &repository,
    ...
    if (auto* static_linear_model = dynamic_cast<const StaticLinearModelDefinition*>(&static_model)) {
...
void DemographicModule::update_population([[maybe_unused]]RuntimeContext &context) {
    ...
    throw std::runtime_error("DemographicModule::update_population(RuntimeContext&) should not be called directly. Use update_population(RuntimeContext&, const DiseaseModule&) instead.");
}
    ...
    for (int age = start_age; age <= end_age; age++) {
        ...
        if (excess_mortality_count.contains(age) && death_rates.contains(age)) {
            ...
            if (std::abs(male_average_product) > 1e-6) {
                male_mortality = 1.0 - (1.0 - death_rates.at(age, core::Gender::male)) / male_average_product;
            } else {
        ...
        if (excess_mortality_count.contains(age) && death_rates.contains(age)) {
            ...
            if (std::abs(female_average_product) > 1e-6) {
                female_mortality = 1.0 - (1.0 - death_rates.at(age, core::Gender::female)) / female_average_product;
            } else {
                female_mortality = death_rates.at(age, core::Gender::female);
            }
            
            // Ensure mortality is in valid range
            residual_mortality.at(age, core::Gender::female) = std::max(std::min(female_mortality, 1.0), 0.0);
        } else {
            // Use small default value if data is missing
