...
std::map<int, DoubleGenderValue> DemographicModule::get_age_gender_distribution(int time_year) const noexcept 
{
    ...
    if (!year_data.empty()) 
    {
        ...
        for (const auto &age : year_data) 
            result.emplace(age.first, DoubleGenderValue(age.second.males * total_ratio, age.second.females * total_ratio));
    }
    return result;
}
    ...
    for (auto entry_count = 1; auto &entry : age_gender_dist) 
    {
        ...
        } else if (pop_diff < 0) 
        {
            ...
            if (entry.second.males > entry.second.females) 
            {
                ...
                if (num_females < 0) num_males += num_females;
            } 
            else 
            {
                num_males -= pop_diff;
...
void DemographicModule::update_population(RuntimeContext &context, const DiseaseModule &disease_host) 
{
    auto residual_future        = core::run_async(&DemographicModule::update_residual_mortality, this, std::ref(context), std::ref(disease_host));
...
void DemographicModule::update_residual_mortality(RuntimeContext &context,  const DiseaseModule &disease_host) 
{
    //// If baseline, calcuate residual mortality afresh
    ...
    { 
        /// If intervention scenario, use residual mortality from baseline scenario.
        ...
        if (message.has_value()) 
        {
            ...
            if (messagePrt) residual_death_rates_ = messagePrt->data();
            else throw std::runtime_error("Simulation out of sync, failed to receive a residual mortality message");
        } 
        else throw std::runtime_error("Simulation out of sync, receive residual mortality message has timed out");
    }
}
...
// For a given person, this function calcuates their contribution to mortality, given their disease status for each disease they may or may not have.
double DemographicModule::calculate_excess_mortality_product(const Person &entity, const DiseaseModule &disease_host) {
    auto product = 1.0;

    // For this Person / entity (pretentious), loop over their diseases. 
    // if they have this diseaes (i.e. status == Active), then get the excess mortality associated with that disease for that...
