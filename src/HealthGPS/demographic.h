...
class DemographicModule final : public SimulationModule {
  ...
  private:
    ...
    std::map<core::Identifier, std::map<core::Gender, std::map<std::string, double>>> region_prevalence_;
    
    /// @brief Ethnicity assignment probabilities by age group, gender, and region
    std::map<core::Identifier, std::map<core::Gender, std::map<std::string, std::map<std::string, double>>>> ethnicity_prevalence_;
    ...
    /// @brief Initialises ethnicity assignment for a person based on age, gender, and region probabilities
    /// @param context The simulation run-time context
