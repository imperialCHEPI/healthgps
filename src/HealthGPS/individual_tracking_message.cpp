...
IndividualTrackingEventMessage::IndividualTrackingEventMessage(std::string sender, unsigned int run,
                                                               int time, std::string scenario_name,
...
std::string IndividualTrackingEventMessage::to_string() const {
    return fmt::format("Source: {}, run # {}, time: {}, scenario: {}, rows: {}", source,
                       run_number, model_time, scenario_name, rows.size());
}
