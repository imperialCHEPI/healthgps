...
namespace hgps {
FiscalPolicyScenario::FiscalPolicyScenario(SyncChannel &data_sync, FiscalPolicyDefinition &&definition)
...
double FiscalPolicyScenario::apply([[maybe_unused]] Random &generator, Person &entity, int time, const core::Identifier &risk_factor_key, double value) 
{
    if (!factor_impact_.contains(risk_factor_key) || !definition_.active_period.contains(time))     return value;

    auto age = entity.age;
    auto &child_effect = definition_.impacts.at(0);
    if (age < child_effect.from_age)     return value;
    ...
    if (age >= adult_effect.from_age) 
    {
        ...
        if (teen_effect.contains(interventions_book_.at(entity.id()))) 
        {
            ...
            if (definition_.impact_type == FiscalImpactType::optimist)     return value;
    ...
    throw std::logic_error(fmt::format("Fiscal intervention error, age {} outside the valid range.", age));
}

const PolicyInterval &FiscalPolicyScenario::active_period() const noexcept {    return definition_.active_period;   }

const std::vector<PolicyImpact> &FiscalPolicyScenario::impacts() const noexcept {    return definition_.impacts;    }

FiscalImpactType parse_fiscal_impact_type(const std::string &impact_type) 
{
    if (core::case_insensitive::equals(impact_type, "pessimist"))   return FiscalImpactType::pessimist;
    if (core::case_insensitive::equals(impact_type, "optimist"))    return FiscalImpactType::optimist;
