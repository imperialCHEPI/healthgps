...
void CachedRepository::register_risk_factor_model_definition(const RiskFactorModelType &model_type, std::unique_ptr<RiskFactorModelDefinition> definition) 
{
    ...
    if (rf_model_definition_.contains(model_type)) rf_model_definition_.erase(model_type);
...
const RiskFactorModelDefinition & CachedRepository::get_risk_factor_model_definition(const RiskFactorModelType &model_type) const 
{
...
DiseaseDefinition &CachedRepository::get_disease_definition(const core::DiseaseInfo &info, const ModelInput &config) 
{
    ...
    if (diseases_.contains(info.code))     return diseases_.at(info.code);
...
void CachedRepository::load_disease_definition(const core::DiseaseInfo &info, const ModelInput &config) 
{
    if (diseases_.contains(info.code)) return;
    ...
    auto relative_risks = detail::create_relative_risk(detail::RelativeRiskInfo{.disease        = info,
    ...
    else 
    {
        ...
        auto definition     = DiseaseDefinition(std::move(disease_table), std::move(relative_risks.diseases), std::move(relative_risks.risk_factors), std::move(parameter));
        diseases_.emplace(info.code, definition);
    }
}
} // namespace hgps
