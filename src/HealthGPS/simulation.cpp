...
Simulation::Simulation(SimulationModuleFactory &factory, std::shared_ptr<const EventAggregator> bus, std::shared_ptr<const ModelInput> inputs, std::unique_ptr<Scenario> scenario)
...
adevs::Time Simulation::init(adevs::SimEnv<int> *env) 
{
    ...
    auto message = fmt::format("[{:4},{}] population size: {}, elapsed: {}ms", env->now().real, env->now().logical, context_.population().initial_size(), elapsed.count());
   
    context_.publish(std::make_unique<InfoEventMessage>(name(), ModelAction::start, context_.current_run(), context_.time_now(), message));
...
adevs::Time Simulation::update(adevs::SimEnv<int> *env) 
{
    if (env->now() < end_time_) 
    {
        ...
        auto message    = fmt::format("[{:4},{}], elapsed: {}ms", env->now().real, env->now().logical, elapsed.count());

        context_.publish(std::make_unique<InfoEventMessage>(name(), ModelAction::update, context_.current_run(), context_.time_now(), message));
...
void Simulation::fini(adevs::Time clock) 
{
    ...
    context_.publish(std::make_unique<InfoEventMessage>(name(), ModelAction::stop, context_.current_run(), context_.time_now(), message));
}
...
void Simulation::update_net_immigration() 
{
    ...
    if (context_.scenario().type() == ScenarioType::baseline) 
        context_.scenario().channel().send(std::make_unique<NetImmigrationMessage>(context_.current_run(), context_.time_now(), std::move(net_immigration)));
}
...
IntegerAgeGenderTable Simulation::get_current_expected_population() const 
{
    ...
    const auto &current_population_table = demographic_->get_population_distribution(context_.time_now());
    ...
    for (int age = start_age; age <= end_age; age++) 
    {
        ...
        expected_population.at(age, core::Gender::male)     = static_cast<int>(std::round(age_info.males    * start_population_size / total_initial_population));
        expected_population.at(age, core::Gender::female)   = static_cast<int>(std::round(age_info.females  * start_population_size / total_initial_population));
    }
...
void Simulation::apply_net_migration(int net_value, unsigned int age, const core::Gender &gender) 
{
    if (net_value > 0)
    {
        ...
        if (!similar_indices.empty()) 
        {
            ...
            {
                auto index = context_.random().next_int(static_cast<int>(similar_indices.size()) - 1);
...
hgps::IntegerAgeGenderTable Simulation::create_net_migration() 
{
    ...
    for (int age = start_age; age <= end_age; age++) 
    {
        net_value = expected_population.at(age, core::Gender::male)     - simulated_population.at(age, core::Gender::male);
        net_emigration.at(age, core::Gender::male) = net_value;

        net_value = expected_population.at(age, core::Gender::female)   - simulated_population.at(age, core::Gender::female);
...
std::map<std::string, core::UnivariateSummary> Simulation::create_input_data_summary() const 
{
    ...
    for (const auto &entry : context_.mapping()) 
        if (const auto &column = input_data.column_if_exists(entry.name()))  // HACK: Ignore missing columns.
        {
...
void hgps::Simulation::print_initial_population_statistics() 
{
    ...
    if (context_.current_run() > 1 && verbosity == core::VerboseMode::none)    return;
    ...
    ss << fmt::format("| {:{}} : {:14} : {:14} : {:14} : {:14} |\n", population, pad, orig_pop, sim_pop, orig_pop, sim_pop);
