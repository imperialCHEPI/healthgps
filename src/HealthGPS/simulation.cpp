...
adevs::Time Simulation::init(adevs::SimEnv<int> *env) {
    ...
    // std::cout << "DEBUG: [Simulation] Simulation start time: " << world_time << ", end time: " <<
    // inputs.stop_time() << ", duration: " << (inputs.stop_time() - world_time) << " years" <<
    // std::endl; std::cout << "DEBUG: [Simulation] ADEVS end_time value: real=" << end_time_.real
...
#pragma warning(pop)
    ...
    // std::cout << "DEBUG: [Simulation] Returning initial time from init: real=" <<
    // initial_time.real << ", logical=" << initial_time.logical << std::endl; std::cout << "DEBUG:
    // [Simulation] Next expected time should be: real=" << (world_time + 1) << ", logical=0" <<
    // std::endl;
...
adevs::Time Simulation::update(adevs::SimEnv<int> *env) {
    ...
    std::cout << "\nDEBUG: [Simulation::update] ====== STARTING UPDATE METHOD ======" << std::endl;
    ...
    if (env->now() < end_time_) {
        // std::cout << "DEBUG: [Simulation::update] Current time (" << env->now().real << ") is
        // less than end time (" << end_time_.real << ")" << std::endl; std::cout << "DEBUG:
        // [Simulation::update] Setting up for year processing" << std::endl;
        auto start = std::chrono::steady_clock::now();
        ...
        // std::cout << "DEBUG: [Simulation::update] Moving world clock to " << time_year <<
        // std::endl;
...
void Simulation::initialise_population() {
    ...
    std::cout << "\n=== STARTING SIMULATION POPULATION INITIALIZATION ===" << std::endl;
    ...
    try {
        ...
        // std::endl; std::cout << "DEBUG: Current simulation time: " << context_.time_now() <<
        // std::endl; std::cout << "DEBUG: Active population size: " <<
        // context_.population().current_active_size() << std::endl; std::cout << "DEBUG: About to
        // proceed to analysis stage" << std::endl;
...
void Simulation::update_population() {
    ...
    try {
        ...
        auto risk_future = std::async(std::launch::async, [this]() {
            try {
                ...
                std::cerr << "CRITICAL ERROR: Unknown exception during risk factor update"
                          << std::endl;
                return false;
            }
        });
    ...
    try {
        ...
        if (!has_active_diseases) {
            std::cout << "WARNING: No active diseases found in population, skipping disease updates"
                      << std::endl;
        } else {
            ...
            auto disease_future = std::async(std::launch::async, [this]() {
                try {
                    ...
                    std::cerr << "CRITICAL ERROR: Unknown exception during disease update"
                              << std::endl;
                    return false;
                }
            });
...
void Simulation::update_net_immigration() {
    try {
        ...
        if (context_.population().current_active_size() == 0) {
            std::cout << "CRITICAL WARNING: Population became inactive after immigration. "
        ...
        if (context_.scenario().type() == ScenarioType::baseline) {
            ...
            for (int attempt = 1; attempt <= max_send_attempts && !send_success; attempt++) {
                try {
                    ...
                    auto target_start_age = context_.age_range().lower();
                    auto target_end_age = context_.age_range().upper();
                    for (int age = target_start_age; age <= target_end_age; age++) {
                        // Check if the keys exist before using at()
                        if (net_immigration.contains(age, core::Gender::male)) {
                            net_migration_copy.at(age, core::Gender::male) =
                                net_immigration.at(age, core::Gender::male);
                        }
                        if (net_immigration.contains(age, core::Gender::female)) {
                            net_migration_copy.at(age, core::Gender::female) =
                                net_immigration.at(age, core::Gender::female);
                        }
                    }
                    ...
                    auto future = std::async(std::launch::async, [&, net_migration_copy]() mutable {
                        try {
                            ...
                            // std::cout << "DEBUG: [Baseline] Sending migration data to
                            // intervention..."  << std::endl;

                            // Create a message and add verification info
                            auto message = std::make_unique<NetImmigrationMessage>(
                                context_.current_run(), context_.time_now(),
                                std::move(net_migration_copy));
                    ...
                    auto timeout_seconds =
                        10 + static_cast<int>(context_.population().initial_size() / 10000);
                    ...
                    if (future.wait_for(std::chrono::seconds(timeout_seconds)) ==
                        ...
                        if (success) {
                            ...
                            std::cerr << "ERROR: Failed to send migration data on attempt "
                                      << attempt << std::endl;
                        }
                    } else {
                        std::cerr << "ERROR: Sending migration data timed out after "
                                  << timeout_seconds << " seconds on attempt " << attempt
                                  << std::endl;
                    }
                    ...
                    std::cerr << "ERROR in baseline send attempt " << attempt << ": " << e.what()
                              << std::endl;
        ...
        // No else clause needed - intervention scenario already received the message in
        // get_net_migration
    } catch (const std::exception &e) {
...
IntegerAgeGenderTable Simulation::get_current_expected_population() const {
    try {
        ...
        if (highest_available_age < start_age) {
            ...
            std::cerr << "CRITICAL ERROR: No valid ages found in population distribution table"
                      << std::endl;
        ...
        for (int age = start_age; age <= end_age; age++) {
            try {
                ...
                if (age <= highest_available_age) {
                    ...
                    expected_population.at(age, core::Gender::male) = static_cast<int>(std::round(
                        age_info.males * start_population_size / total_initial_population));
                    expected_population.at(age, core::Gender::female) = static_cast<int>(std::round(
                        age_info.females * start_population_size / total_initial_population));
                }
                // For ages beyond the available data, use the highest age data exactly as is
                else {
                    const auto &highest_age_info =
                        current_population_table.at(highest_available_age);

                    expected_population.at(age, core::Gender::male) = static_cast<int>(std::round(
                        highest_age_info.males * start_population_size / total_initial_population));
                ...
                if (age > start_age) {
                    ...
                    expected_population.at(age, core::Gender::male) =
                        expected_population.at(age - 1, core::Gender::male) / 2;
                    expected_population.at(age, core::Gender::female) =
                        expected_population.at(age - 1, core::Gender::female) / 2;
                } else {
                    // Minimal fallback
...
IntegerAgeGenderTable Simulation::get_net_migration() {
    ...
    if (context_.scenario().type() == ScenarioType::baseline) {
        try {
            ...
            // migration data with zeros" << std::endl;
    ...
    // std::cout<< "DEBUG: [get_net_migration] Intervention scenario - attempting to receive
    ...
    try {
        ...
        double size_factor =
            std::min(40.0, std::max(5.0, static_cast<double>(context_.population().initial_size()) /
                                             2000.0));
        ...
        for (int attempt = 0; attempt < max_attempts; attempt++) {
            ...
            auto wait_time =
                static_cast<int>(std::min(raw_wait_time, 2.0 * 1000 * 1000)); // Max 2000 seconds
            ...
            if (context_.time_now() == context_.start_time()) {
                if (attempt == 0) {
                    ...
                    wait_time = static_cast<int>(
                        std::min(15000.0 * size_factor, 2.0 * 1000 * 1000)); // Max 2000 seconds
                } else {
                    // For subsequent attempts in first year, use longer backoff
                    raw_wait_time = 4000.0 * size_factor * backoff_factor;
                    wait_time = static_cast<int>(
                        std::min(raw_wait_time, 2.0 * 1000 * 1000)); // Max 2000 seconds
                }
            }
                    ...
                    try {
                        ...
                        if (messagePtr) {
                            try {
                                ...
                                // NetImmigrationMessage type" << std::endl;
                                ...
                                std::cerr << "ERROR accessing message data: " << e.what()
                                          << std::endl;
                            }
                        } else {
                            std::cerr << "WARNING: Received wrong message type on attempt "
                                      << (attempt + 1) << std::endl;
                        }
                    } catch (const std::exception &e) {
                        std::cerr << "WARNING: Error processing received message: " << e.what()
                                  << std::endl;
                    }
                } else {
                    std::cout << "DEBUG: [get_net_migration] No message received after timeout"
                              << std::endl;
                }
            } catch (const std::exception &e) {
                std::cerr << "WARNING: Receive attempt " << (attempt + 1) << " failed: " << e.what()
                          << std::endl;
        ...
        std::cout << "DEBUG: [get_net_migration] All attempts failed, using fallback data"
                  << std::endl;
    } catch (const std::exception &e) {
...
IntegerAgeGenderTable Simulation::create_net_migration() {
    ...
    try {
        ...
        auto sim_pop_future = std::async(
            std::launch::async, [this]() { return this->get_current_simulated_population(); });
        ...
        // std::cout << "DEBUG: [create_net_migration] Successfully retrieved simulated
        // population"<< std::endl;
        ...
        if (!simulated_valid) {
            std::cerr
                << "WARNING: Simulated population data is incomplete. Using zeros for migration."
                << std::endl;
        ...
        try {
            auto expected_future = std::async(
                std::launch::async, [this]() { return this->get_current_expected_population(); });
            ...
            // std::cout << "DEBUG: [create_net_migration] Successfully retrieved expected
            // population" << std::endl;
        } catch (const std::exception &e) {
        ...
        if (!expected_valid) {
            std::cerr
                << "WARNING: Expected population data is incomplete. Using zeros for migration."
                << std::endl;
            return net_emigration; // Return zeros
        }

        // std::cout << "DEBUG: [create_net_migration] Calculating migration differences" <<
        // std::endl;
        ...
        for (int age = start_age; age <= end_age; age++) {
            try {
                ...
                std::cerr << "Error calculating male migration for age " << age << ": " << e.what()
                          << std::endl;
                // Keep value as 0
            }
            ...
            try {
                ...
                if (expected_population.contains(age, core::Gender::female) &&
                    ...
                    // Calculate net migration
                    net_emigration.at(age, core::Gender::female) =
                        expected_females - simulated_females;
                }
            } catch (const std::exception &e) {
                std::cerr << "Error calculating female migration for age " << age << ": "
                          << e.what() << std::endl;
                // Keep value as 0
            }
        }
