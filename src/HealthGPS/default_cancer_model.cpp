...
DefaultCancerModel::DefaultCancerModel(DiseaseDefinition &definition, WeightModel &&classifier, const core::IntegerInterval &age_range)
...
{ 
    return core::DiseaseGroup::cancer; 
}
...
void DefaultCancerModel::initialise_disease_status(RuntimeContext &context) 
{
    ...
    for (auto &person : context.population()) 
    {
        ...
        if (hazard < probability) 
        {
            ...
            person.diseases[disease_type()] = Disease{.status = DiseaseStatus::active, .start_time = 0, .time_since_onset = time_since_onset};
        }
    }
}
