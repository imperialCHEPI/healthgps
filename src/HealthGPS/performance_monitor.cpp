...
void PerformanceMonitor::print_summary() const {
    ...
    std::vector<std::pair<std::string, OperationMetrics>> sorted_ops(operations_.begin(), operations_.end());
    ...
    for (const auto& [name, metrics] : sorted_ops) {
        double max_time = metrics.times.empty() ? 0.0 : 
                         *std::max_element(metrics.times.begin(), metrics.times.end());
        
        fmt::print("{:<30} {:<10} {:<12.2f} {:<12.2f} {:<12.2f}\n",
                   name, metrics.count, metrics.total_time, metrics.average(), max_time);
    }
