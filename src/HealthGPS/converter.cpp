...
{
    if (core::case_insensitive::equals(name, "male"))   return core::Gender::male;
    if (core::case_insensitive::equals(name, "female")) return core::Gender::female;
...
FloatAgeGenderTable StoreConverter::to_relative_risk_table(const core::RelativeRiskEntity &entity) 
{
    ...
    for (size_t i = 1; i <= num_cols; i++) 
    {
        ...
        if (gender == core::Gender::unknown) 
            throw std::out_of_range(fmt::format("Invalid column gender type: {}", entity.columns[i]));
...
RelativeRisk create_relative_risk(const RelativeRiskInfo &info) 
{
    ...
    {
        if (const auto table_male = info.manager.get_relative_risk_to_risk_factor(info.disease, core::Gender::male, factor.key())) 
            result.risk_factors[factor.key()].emplace(core::Gender::male, StoreConverter::to_relative_risk_lookup(*table_male));

        if (const auto table_female = info.manager.get_relative_risk_to_risk_factor(info.disease, core::Gender::female, factor.key())) 
            result.risk_factors[factor.key()].emplace(core::Gender::female, StoreConverter::to_relative_risk_lookup(*table_female));
    }
...
LifeTable StoreConverter::to_life_table(const std::vector<core::BirthItem> &births, const std::vector<core::MortalityItem> &deaths) 
{
...
LmsDefinition StoreConverter::to_lms_definition(const std::vector<core::LmsDataRow> &dataset) 
{
    ...
    for (const auto &row : dataset) 
        lms_dataset[row.age][row.gender] = LmsRecord{.lambda = row.lambda, .mu = row.mu, .sigma = row.sigma};
