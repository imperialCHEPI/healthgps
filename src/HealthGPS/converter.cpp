...
std::map<int, std::map<int, PopulationRecord>> StoreConverter::to_population_records(const std::vector<core::PopulationItem> &items) {
    ...
    for (const auto &item : items) {
        ...
        result.at(item.at_time).emplace(item.with_age, PopulationRecord(item.with_age, item.males, item.females));
    }
