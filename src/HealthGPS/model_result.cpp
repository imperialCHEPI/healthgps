...
ModelResult::ModelResult(const unsigned int sample_size) : series{sample_size} {}
...
std::string ModelResult::to_string() const noexcept {
    ...
    ss << fmt::format("Population.: {}, alive = {}, migrating = {}, dead = {}\n", population_size,
                      number_alive.total(), number_emigrated, number_dead);

    ss << fmt::format("Average Age: Male = {:.5f}, Female = {:.5f}\n", average_age.male,
                      average_age.female);

    ss << fmt::format("Indicators.: YLL= {:.5f}, YLD = {:.5f}, DALY = {:.5f}\n",
                      indicators.years_of_life_lost, indicators.years_lived_with_disability,
