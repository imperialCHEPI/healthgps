...
{ 
    return core::DiseaseGroup::other; 
}
...
void DefaultDiseaseModel::initialise_disease_status(RuntimeContext &context) 
{
    ...
    for (auto &person : context.population()) 
    {
        ...
        if (hazard < probability) 
        {
            ...
            person.diseases[disease_type()] = Disease{.status = DiseaseStatus::active, .start_time = 0};
        }
    }
}
...
void DefaultDiseaseModel::initialise_average_relative_risk(RuntimeContext &context) 
{
    ...
    for (int age = age_range.lower(); age <= age_range.upper(); age++) 
    {
        ...
        if (sum.contains(age)) 
        {
            ...
            if (male_count > 0.0)   male_average    = sum(age, core::Gender::male)      / male_count;
            if (female_count > 0.0) female_average  = sum(age, core::Gender::female)    / female_count;
        }
...
void DefaultDiseaseModel::update_disease_status(RuntimeContext &context) 
{
 ...
 // gets The excess mortality for a given person's age and sex
double DefaultDiseaseModel::get_excess_mortality(const Person &person) const noexcept 
{
    // MeasureKey::mortality is essentially just a string. 
    // all of this is a way of saying that this function wants the mortality part of whatever table is returned by the 
    //// (again convoulted) definition_.get().table()(person.age, person.gender).at(mortality_id); line.
...
double DefaultDiseaseModel::calculate_relative_risk_for_risk_factors(const Person &person) const 
{
    ...
    {
        if (!relative_risk_tables.contains(factor_name)) continue;

        auto factor_value_adjusted = static_cast<float>(weight_classifier_.adjust_risk_factor_value(person, factor_name, factor_value));
...
void DefaultDiseaseModel::update_remission_cases(RuntimeContext &context) 
{
    ...
    for (auto &person : context.population()) 
    {
        ...
        auto hazard         = context.random().next_double(); // not a hazard
        if (hazard < probability) person.diseases.at(disease_type()).status = DiseaseStatus::free;
    }
}
...
void DefaultDiseaseModel::update_incidence_cases(RuntimeContext &context) 
{
    ...
    for (auto &person : context.population()) 
    {
        ...
        if (hazard < probability) 
            person.diseases[disease_type()] = Disease{.status = DiseaseStatus::active, .start_time = context.time_now()};
    }
}
