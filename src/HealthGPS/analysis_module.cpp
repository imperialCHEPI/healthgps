...
AnalysisModule::AnalysisModule(AnalysisDefinition &&definition, WeightModel &&classifier, const core::IntegerInterval age_range, unsigned int comorbidities)
...
AnalysisModule::AnalysisModule(AnalysisDefinition &&definition, WeightModel &&classifier, const core::IntegerInterval age_range, unsigned int comorbidities, std::vector<double> calculated_stats)
...
void AnalysisModule::initialise_vector(RuntimeContext &context) 
{
    ...
    {
        const auto [min, max] = std::ranges::minmax_element(context.population(), 
            [&factor](const auto &entity1, const auto &entity2) 
            {
    ...
    size_t total_num_bins = std::accumulate(factor_bins_.cbegin(), factor_bins_.cend(), size_t{1}, std::multiplies<>());
    ...
    tbb::parallel_for_each(pop.cbegin(), pop.cend(), [&](const auto &entity) 
        {
            ...
            expected_sum(entity.age, entity.gender) += Product;
            expected_count(entity.age, entity.gender)++;
         });

    //// then use the above expected_sum and expected_count variables to create residual_disability_weight_
    for (int age = age_range.lower(); age <= age_range.upper(); age++) 
    {
        residual_disability_weight_(age, core::Gender::male  ) = calculate_residual_disability_weight(age, core::Gender::male  , expected_sum, expected_count);
        residual_disability_weight_(age, core::Gender::female) = calculate_residual_disability_weight(age, core::Gender::female, expected_sum, expected_count);
    }
...
double AnalysisModule::calculate_residual_disability_weight(int age, const core::Gender gender, const DoubleAgeGenderTable &expected_sum, const IntegerAgeGenderTable &expected_count) 
{
    ...
    if (denominator != 0.0) 
    {
        ...
        if (std::isnan(residual_value)) residual_value = 0.0;
    }
...
void AnalysisModule::calculate_historical_statistics(RuntimeContext &context, ModelResult &result) const 
{
    ...
    for (const auto &entity : context.population()) 
    {
        if (!entity.is_active()) 
        {
            if (entity.has_emigrated() && entity.time_of_migration() == analysis_time)    population_migrated++;
            if (!entity.is_alive() && entity.time_of_death() == analysis_time)     population_dead++;
        ...
        for (auto &item : risk_factors) 
        {
            ...
            if (std::isnan(factor_value))     factor_value = 0.0;
        ...
        if (comorbidity_number > comorbidities_)    comorbidity_number = comorbidities_;

        if (entity.gender == core::Gender::male)    comorbidity[comorbidity_number].male++;
        else                                        comorbidity[comorbidity_number].female++;
    }
...
{
    size_t num_factors_to_calculate = context.mapping().entries().size() - factors_to_calculate_.size();
    ...
    for (const auto &person : context.population()) 
    {
        ...
        for (size_t i = 0; i < factors_to_calculate_.size(); i++) 
        {
            ...
            auto bin_index      = static_cast<size_t>((factor_value - factor_min_values_[i]) / factor_bin_widths_[i]);
        ...
        {
            size_t accumulated_bins = std::accumulate(std::next(factor_bins_.cbegin(), i + 1), factor_bins_.cend(), size_t{1}, std::multiplies<>());
...
// NOLINTBEGIN(readability-function-cognitive-complexity)
void AnalysisModule::calculate_population_statistics(RuntimeContext &context, DataSeries &series) const 
{
    if (series.size() > 0)    throw std::logic_error("This should be a new object!");
    ...
    for (const auto &person : context.population()) 
    {
        ...
        if (!person.is_active()) 
        {
            ...
            if (person.has_emigrated() && person.time_of_migration() == current_time)   series(gender, "emigrations").at(age)++;
        ...
        for (const auto &factor : context.mapping().entries()) 
            series(gender, "mean_" + factor.key().to_string()).at(age) += person.get_risk_factor_value(factor.key());
...
void AnalysisModule::calculate_standard_deviation(RuntimeContext &context, DataSeries &series) const 
{
    // Accumulate squared deviations from mean.
    auto accumulate_squared_diffs = 
        [&series](const std::string &chan, core::Gender sex, int age, double value) 
        {
    ...
    auto divide_by_count_sqrt = [&series](const std::string &chan, core::Gender sex, int age, double count) 
        {
