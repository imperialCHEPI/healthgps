...
class PerformanceOptimizer {
  public:
    ...
    /// @param population The population to process
    /// @param func Function to apply to each person
    template <typename Function> void age_grouped_process(Population &population, Function &&func);
...
void PerformanceOptimizer::parallel_process_active(Population &population, Function &&func) {
    ...
    if (config_.enable_cache_optimization) {
        ...
        tbb::parallel_for_each(
            active_people.begin(), active_people.end(),
            [&func](std::reference_wrapper<Person> &person_ref) { func(person_ref.get()); });
    } else {
        // Standard parallel_for_each
        tbb::parallel_for_each(population.begin(), population.end(), [&func](Person &person) {
            if (person.is_active()) {
                func(person);
            }
        });
...
void PerformanceOptimizer::parallel_process_blocked(Population &population, Function &&func) {
    ...
    tbb::parallel_for(tbb::blocked_range<Population::iterator>(population.begin(), population.end(),
                                                               config_.chunk_size),
...
void PerformanceOptimizer::age_grouped_process(Population &population, Function &&func) {
    ...
    tbb::parallel_for_each(age_groups.begin(), age_groups.end(), [&func](auto &age_group) {
...
class OptimizedStatsCalculator {
  public:
    ...
    static std::map<std::string, double> calculate_stats(Population &population,
    ...
    static std::map<core::Identifier, std::map<std::string, double>>
    calculate_risk_factor_stats(Population &population,
