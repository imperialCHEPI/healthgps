...
{
    if (model_name == "dummy")          return DummyModelSchemaVersion;
    if (model_name == "ebhlm")          return EBHLMModelSchemaVersion;
    if (model_name == "kevinhall")      return KevinHallModelSchemaVersion;
    if (model_name == "hlm")            return HLMModelSchemaVersion;
    if (model_name == "staticlinear")   return StaticLinearModelSchemaVersion;
...
std::pair<std::string, nlohmann::json> load_and_validate_model_json(const std::filesystem::path &model_path) 
{
    std::ifstream ifs{model_path};
    if (!ifs) throw std::runtime_error(fmt::format("Could not read file: {}", model_path.string()));
...
std::unique_ptr<hgps::RiskFactorSexAgeTable> load_risk_factor_expected(const Configuration &config) 
{
    ...
    try 
    {
        table->emplace_row(hgps::core::Gender::male,    load_baseline_from_csv(male_filename, info.delimiter));
        table->emplace_row(hgps::core::Gender::female,  load_baseline_from_csv(female_filename, info.delimiter));
    } 
    catch (const std::runtime_error &ex) 
    {
        throw hgps::core::HgpsException{fmt::format("Failed to parse adjustment file: {} or {}. {}", male_filename, female_filename, ex.what())};
    }
    ...
    for (const auto &sex : *table) 
        for (const auto &factor : sex.second) 
            if (factor.second.size() <= max_age) 
                throw hgps::core::HgpsException{fmt::format("Baseline adjustment file must cover the required age range: [{}].", config.settings.age_range.to_string())};
...
std::unique_ptr<hgps::DummyModelDefinition> load_dummy_risk_model_definition(hgps::RiskFactorModelType type, const nlohmann::json &opt) 
{
    ...
    return std::make_unique<hgps::DummyModelDefinition>(type, std::move(names), std::move(values), std::move(policy), std::move(policy_start));
}
...
std::unique_ptr<hgps::StaticHierarchicalLinearModelDefinition> load_hlm_risk_model_definition(const nlohmann::json &opt) 
{
    ...
    for (const auto &model_item : model_info.models) 
    {
        ...
        models.emplace( hgps::core::Identifier(model_item.first),
    ...
    for (auto &level_item : model_info.levels) 
    {
        ...
        levels.emplace(
            ...
            hgps::HierarchicalLevel{
                ...
                .transition             = hgps::core::DoubleArray2D(at.transition.rows              , at.transition.cols            , at.transition.data            ),
                .inverse_transition     = hgps::core::DoubleArray2D(at.inverse_transition.rows      , at.inverse_transition.cols    , at.inverse_transition.data    ),
                .residual_distribution  = hgps::core::DoubleArray2D(at.residual_distribution.rows   , at.residual_distribution.cols , at.residual_distribution.data ),
                .correlation            = hgps::core::DoubleArray2D(at.correlation.rows             , at.correlation.cols           , at.correlation.data           ),
                .variances              = at.variances});
    }
    return std::make_unique<hgps::StaticHierarchicalLinearModelDefinition>(std::move(models), std::move(levels));
}
...
std::unique_ptr<hgps::StaticLinearModelDefinition> load_staticlinear_risk_model_definition(const nlohmann::json &opt, const Configuration &config) 
{
    ...
    const auto correlation_file_info    = input::get_file_info(opt["RiskFactorCorrelationFile"], config.root_path);
    ...
    const auto policy_covariance_file_info  = input::get_file_info(opt["PolicyCovarianceFile"], config.root_path);
    const auto policy_covariance_table      = load_datatable_from_csv(policy_covariance_file_info);
    Eigen::MatrixXd policy_covariance{policy_covariance_table.num_rows(), policy_covariance_table.num_columns()};
    ...
    for (const auto &[key, json_params] : opt["RiskFactorModels"].items()) 
    {
        ...
        model.coefficients  = json_params["Coefficients"].get<std::unordered_map<core::Identifier, double>>();
        ...
        policy_model.coefficients       = policy_json_params["Coefficients"].get<std::unordered_map<core::Identifier, double>>();
        policy_model.log_coefficients   = policy_json_params["LogCoefficients"].get<std::unordered_map<core::Identifier, double>>();
        ...
        for (size_t j = 0; j < policy_covariance_table.num_rows(); j++) 
            policy_covariance(i, j) = std::any_cast<double>(policy_covariance_table.column(i).value(j));
        ...
        trend_model.coefficients        = trend_json_params["Coefficients"].get<std::unordered_map<core::Identifier, double>>();
        trend_model.log_coefficients    = trend_json_params["LogCoefficients"] .get<std::unordered_map<core::Identifier, double>>();
    ...
    for (const auto &name : names) 
    {
        if (!expected->at(core::Gender::male).contains(name)) 
            throw core::HgpsException{fmt::format("'{}' is not defined in male risk factor expected values.", name.to_string())};
        if (!expected->at(core::Gender::female).contains(name)) 
            throw core::HgpsException{fmt::format("'{}' is not defined in female risk factor expected values.", name.to_string())};
    }
    ...
    for (const auto &age_group : opt["RuralPrevalence"]) 
    {
        ...
        rural_prevalence[age_group_name]    = {{core::Gender::female, age_group["Female"]}, {core::Gender::male, age_group["Male"]}};
    }
    ...
    for (const auto &[key, json_params] : opt["IncomeModels"].items()) 
    {
        ...
        if (core::case_insensitive::equals(key, "Unknown"))     category = core::Income::unknown;
        else if (core::case_insensitive::equals(key, "Low"))    category = core::Income::low;
        else if (core::case_insensitive::equals(key, "Middle")) category = core::Income::middle;
        else if (core::case_insensitive::equals(key, "High"))   category = core::Income::high;
        else throw core::HgpsException(fmt::format("Income category {} is unrecognised.", key));
...
std::unique_ptr<hgps::DynamicHierarchicalLinearModelDefinition> load_ebhlm_risk_model_definition(const nlohmann::json &opt, const Configuration &config) 
{
    ...
    if (info.percentage > 0.0 && info.percentage < 1.0)     percentage = info.percentage;
    else throw hgps::core::HgpsException{fmt::format("Boundary percentage outside range (0, 1): {}", info.percentage)};
    ...
    for (const auto &age_grp : info.equations) 
    {
        ...
        auto age_key        = hgps::core::IntegerInterval(std::stoi(limits[0].data()), std::stoi(limits[1].data()));
        ...
        for (const auto &gender : age_grp.second) 
        {
            ...
            if (hgps::core::case_insensitive::equals("male", gender.first)) 
            {
                for (const auto &func : gender.second) 
                {
                    ...
                    for (const auto &coeff : func.coefficients) 
                        function.coefficients.emplace(hgps::core::to_lower(coeff.first), coeff.second);
            ...
            else throw hgps::core::HgpsException{fmt::format("Unknown model gender type: {}", gender.first)};
        }
    ...
    return std::make_unique<hgps::DynamicHierarchicalLinearModelDefinition>(
        std::move(expected), std::move(expected_trend), std::move(trend_steps), std::move(equations), std::move(variables), percentage);
}
...
std::unique_ptr<hgps::KevinHallModelDefinition> load_kevinhall_risk_model_definition(const nlohmann::json &opt, const Configuration &config) 
{
    ...
    // Weight quantiles.
    const auto weight_quantiles_table_F = load_datatable_from_csv(hgps::input::get_file_info(opt["WeightQuantiles"]["Female"]   , config.root_path));
    const auto weight_quantiles_table_M = load_datatable_from_csv(hgps::input::get_file_info(opt["WeightQuantiles"]["Male"]     , config.root_path));

    std::unordered_map<hgps::core::Gender, std::vector<double>> weight_quantiles = {    {hgps::core::Gender::female , {}}, 
    ...
    for (size_t j = 0; j < weight_quantiles_table_F.num_rows(); j++) 
        weight_quantiles[hgps::core::Gender::female].push_back(std::any_cast<double>(weight_quantiles_table_F.column(0).value(j)));
    
    for (size_t j = 0; j < weight_quantiles_table_M.num_rows(); j++) 
        weight_quantiles[hgps::core::Gender::male].push_back(std::any_cast<double>(weight_quantiles_table_M.column(0).value(j)));
    
    for (auto &[unused, quantiles] : weight_quantiles) std::sort(quantiles.begin(), quantiles.end());

    // Energy Physical Activity quantiles.
    const auto epa_quantiles_table = load_datatable_from_csv(hgps::input::get_file_info(opt["EnergyPhysicalActivityQuantiles"], config.root_path));
    ...
    std::unordered_map<hgps::core::Gender, double> height_stddev = {    {hgps::core::Gender::female , opt["HeightStdDev"]["Female"  ].get<double>()},
                                                                        {hgps::core::Gender::male   , opt["HeightStdDev"]["Male"    ].get<double>()}    };
    
    std::unordered_map<hgps::core::Gender, double> height_slope = {     {hgps::core::Gender::female , opt["HeightSlope"]["Female"   ].get<double>()},
    ...
    return std::make_unique<hgps::KevinHallModelDefinition>(
        ...
        std::move(energy_equation), 
        std::move(nutrient_ranges), 
        std::move(nutrient_equations),
        std::move(food_prices), 
        std::move(weight_quantiles), 
        std::move(epa_quantiles),
        std::move(height_stddev), 
...
std::unique_ptr<hgps::RiskFactorModelDefinition> load_risk_model_definition(hgps::RiskFactorModelType model_type, const std::filesystem::path &model_path, const Configuration &config) 
{
    ...
    if (model_name == "dummy")     return load_dummy_risk_model_definition(model_type, opt);

    switch (model_type) {
    case hgps::RiskFactorModelType::Static:
        // Load this static model with the appropriate loader.
        if (model_name == "hlm")            return load_hlm_risk_model_definition           (opt);
        if (model_name == "staticlinear")   return load_staticlinear_risk_model_definition  (opt, config);
        throw hgps::core::HgpsException{fmt::format("Static model name '{}' not recognised", model_name)};
    
    case hgps::RiskFactorModelType::Dynamic:
        // Load this dynamic model with the appropriate loader.
        if (model_name == "ebhlm")      return load_ebhlm_risk_model_definition     (opt, config);
        if (model_name == "kevinhall")  return load_kevinhall_risk_model_definition (opt, config);
        throw hgps::core::HgpsException{fmt::format("Dynamic model name '{}' is not recognised.", model_name)};
...
void register_risk_factor_model_definitions(hgps::CachedRepository &repository, const Configuration &config)
{
    ...
    for (const auto &[model_type_str, model_path] : config.modelling.risk_factor_models) 
    {
        ...
        if (model_type_str == "static")         model_type = RiskFactorModelType::Static;
        else if (model_type_str == "dynamic")   model_type = RiskFactorModelType::Dynamic;
        else throw hgps::core::HgpsException(fmt::format("Unknown risk factor model type: {}", model_type_str));
