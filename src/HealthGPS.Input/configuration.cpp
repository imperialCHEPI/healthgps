...
std::unique_ptr<DataSource> get_data_source_from_json(const nlohmann::json &opt, const std::filesystem::path &root_path)
{
    ...
    if (opt.contains("checksum")) 
    {
        ...
        return std::make_unique<ValidatedDataSource>(std::move(source), root_path, std::move(file_hash));
    }
...
std::filesystem::path get_config_file_path(const std::string &config_source) 
{
    ...
    if (std::filesystem::is_regular_file(config_source) && config_source.ends_with(".json")) return config_source;
...
Configuration get_configuration(const std::string &config_source, const std::optional<std::string> &output_folder, int job_id, bool verbose) 
{
    ...
    if (verbose)  config.verbosity = core::VerboseMode::verbose;

    const auto config_file  = get_config_file_path(config_source);
    const auto opt          = load_and_validate_json(config_file, ConfigSchemaFileName, ConfigSchemaVersion, /*require_schema_property=*/ false);
    ...
    // mandatory.
    if (opt.contains("data")) config.data_source = get_data_source_from_json(opt["data"], config.root_path);
    ...
    if (!success) throw ConfigurationError{"Error loading config file"};
...
ModelInput create_model_input(core::DataTable &input_table, core::Country country, const Configuration &config, std::vector<core::DiseaseInfo> diseases) 
{
    ...
    auto settings           = Settings(std::move(country), config.settings.size_fraction, config.settings.age_range);
    ...
    auto ses_mapping = SESDefinition{.fuction_name = config.ses.function, .parameters = config.ses.parameters};
...
std::string create_output_file_name(const OutputInfo &info, int job_id)
{
    ...
    if (tk_end > 0) log_file_name = file_name;

    if (job_id > 0) 
    {
        tk_start = log_file_name.find_last_of('.');
        if (tk_start != std::string::npos)  log_file_name.replace(tk_start, size_t{1}, fmt::format("_{}.", std::to_string(job_id)));
        else                                log_file_name.append(fmt::format("_{}.json", std::to_string(job_id)));
...
std::unique_ptr<hgps::Scenario> create_intervention_scenario(SyncChannel &channel, const PolicyScenarioInfo &info) 
{
    using namespace hgps;
    ...
    for (const auto &item : info.impacts) 
        risk_impacts.emplace_back(core::Identifier{item.risk_factor}, item.impact_value, item.from_age, item.to_age);
    ...
    throw std::invalid_argument(fmt::format("Unknown intervention policy identifier: {}", info.identifier));
}
