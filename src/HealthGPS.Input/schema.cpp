...
void validate_json(std::istream &is, const std::string &schema_file_name, int schema_version) 
{
    ...
    const auto schema_relative_path = std::filesystem::path{"schemas"} / fmt::format("v{}", schema_version) / schema_file_name;
    
    auto ifs_schema = std::ifstream{program_dir / schema_relative_path};
    if (!ifs_schema) throw std::runtime_error("Failed to load schema");
...
nlohmann::json load_and_validate_json(const std::filesystem::path &file_path, const std::string &schema_file_name, int schema_version, bool require_schema_property) 
{
    ...
    if (!ifs) throw std::runtime_error(fmt::format("File not found: {}", file_path.string()));
    ...
    if (!json.contains("$schema")) 
    {
        ...
        if (require_schema_property)    throw std::runtime_error(message);
        else                            fmt::print(fmt::fg(fmt::color::dark_salmon), "{}\n", message);
    ...
    else 
    {
        ...
        const auto expected_schema_url  = fmt::format("{}v{}/{}", SchemaURLPrefix, schema_version, schema_file_name);
        if (actual_schema_url != expected_schema_url) 
            throw std::runtime_error(fmt::format("Invalid schema URL provided: {} (expected: {})", actual_schema_url, expected_schema_url));
    }
