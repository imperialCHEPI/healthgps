...
TEST(JsonParser, OptionalIntervalFromJson) {
    {
        // Successful parsing of non-null interval
    ...
    {
        // Successful parsing of null JSON value
        std::optional<IntegerInterval> interval;
        const auto j = json{};
        EXPECT_NO_THROW(j.get_to(interval));
    ...
    {
        // Successful parsing of empty JSON array (equivalent to null)
        std::optional<IntegerInterval> interval;
        const auto j = json::array();
        EXPECT_NO_THROW(j.get_to(interval));
        EXPECT_FALSE(interval.has_value());
...
TEST(JsonParser, OptionalIntervalToJson) {
    {
        // Null interval
        const std::optional<IntegerInterval> interval;
        const json j = interval;
        EXPECT_EQ(j.get<decltype(interval)>(), interval);
    }

    {
        // Non-null interval
        const auto interval = std::make_optional<IntegerInterval>(0, 1);
        const json j = interval;
        EXPECT_EQ(j.get<decltype(interval)>(), interval);
    }
}