...
protected:
    ...
    void create_test_population_data() {
        ...
        for (int year = 2020; year <= 2025; year++) {
            ...
            for (int age = 0; age <= 100; age++) {
                ...
                year_data[age] = PopulationRecord(age, static_cast<float>(males), static_cast<float>(females));
            }
    ...
    void create_test_life_table() {
        ...
        for (int year = 2020; year <= 2025; year++) {
            births.push_back(BirthIndicator(year, 1000.0 + year * 10, 1.05)); // More boys than girls
        }
        ...
        for (int year = 2020; year <= 2025; year++) {
            for (int age = 0; age <= 100; age++) {
                ...
                deaths.push_back(MortalityIndicator(year, age, static_cast<float>(male_rate), static_cast<float>(female_rate)));
            }
        }
...
TEST_F(DemographicModuleTest, Constructor) {
    auto module = std::make_unique<DemographicModule>(
        std::move(pop_data_), std::move(life_table_));
...
TEST_F(DemographicModuleTest, ConstructorWithEmptyPopulationData) {
    ...
    auto module = std::make_unique<DemographicModule>(
        std::move(empty_pop_data), std::move(empty_life_table));
...
TEST_F(DemographicModuleTest, ConstructorWithMismatchedData) {
    ...
    EXPECT_THROW(
        DemographicModule(std::move(empty_pop_data), std::move(non_empty_life_table)),
...
TEST_F(DemographicModuleTest, ConstructorWithEmptyLifeTable) {
    ...
    EXPECT_THROW(
        DemographicModule(std::move(non_empty_pop_data), std::move(empty_life_table)),
        std::invalid_argument);
}
...
TEST_F(DemographicModuleTest, ConstructorWithTimeRangeMismatch) {
    ...
    EXPECT_THROW(
        DemographicModule(std::move(pop_data), std::move(life_table)),
        std::invalid_argument);
}
...
TEST_F(DemographicModuleTest, CalculateResidualMortality) {
    ...
    auto residual_mortality = module->calculate_residual_mortality(*mock_context, *mock_disease_module);
