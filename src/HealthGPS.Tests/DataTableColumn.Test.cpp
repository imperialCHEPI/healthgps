...
TEST(DataTableColumnTest, IntegerDataTableColumnNameValidation) {
    ...
    ASSERT_THROW(IntegerDataTableColumn("invalid name", std::vector<int>{1, 2, 3}), std::invalid_argument);
    ASSERT_THROW(IntegerDataTableColumn("invalid-name", std::vector<int>{1, 2, 3}), std::invalid_argument);
    ASSERT_THROW(IntegerDataTableColumn("invalid@name", std::vector<int>{1, 2, 3}), std::invalid_argument);
    ASSERT_THROW(IntegerDataTableColumn("invalid#name", std::vector<int>{1, 2, 3}), std::invalid_argument);
    ASSERT_THROW(IntegerDataTableColumn("invalid$name", std::vector<int>{1, 2, 3}), std::invalid_argument);
...
public:
    ...
    void visit(const StringDataTableColumn& /* column */) override {
        visited_type = "string";
    }
    
    void visit(const IntegerDataTableColumn& /* column */) override {
        visited_type = "integer";
    }
    
    void visit(const FloatDataTableColumn& /* column */) override {
        visited_type = "float";
    }
    
    void visit(const DoubleDataTableColumn& /* column */) override {
        visited_type = "double";
    }
};
