...
TEST(TestHealthGPS_Queue, CreateUniquePtrQueue) {
    using namespace hgps;
    using namespace ::std::literals;

    auto q = ThreadsafeQueue<std::unique_ptr<int>>{};
    auto result = std::vector<std::unique_ptr<int>>{};
    ...
    auto consumer = std::jthread([&q, &result](std::stop_token token) {
        while (true) {
            ...
            result.push_back(std::move(v.value()));
            if (token.stop_requested()) {
                break;
            }
        }
