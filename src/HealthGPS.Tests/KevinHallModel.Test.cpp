...
TEST_F(KevinHallModelTest, Constructor) {
    auto model = std::make_unique<KevinHallModel>(
        expected_, expected_trend_, trend_steps_,
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyEnergyEquation) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            expected_, expected_trend_, trend_steps_,
            empty_energy_equation, nutrient_ranges_, nutrient_equations_,
            food_prices_, weight_quantiles_, epa_quantiles_,
            height_stddev_, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyNutrientRanges) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            expected_, expected_trend_, trend_steps_,
            energy_equation_, empty_nutrient_ranges, nutrient_equations_,
            food_prices_, weight_quantiles_, epa_quantiles_,
            height_stddev_, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyNutrientEquations) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            expected_, expected_trend_, trend_steps_,
            energy_equation_, nutrient_ranges_, empty_nutrient_equations,
            food_prices_, weight_quantiles_, epa_quantiles_,
            height_stddev_, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyFoodPrices) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            expected_, expected_trend_, trend_steps_,
            energy_equation_, nutrient_ranges_, nutrient_equations_,
            empty_food_prices, weight_quantiles_, epa_quantiles_,
            height_stddev_, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyWeightQuantiles) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            expected_, expected_trend_, trend_steps_,
            energy_equation_, nutrient_ranges_, nutrient_equations_,
            food_prices_, empty_weight_quantiles, epa_quantiles_,
            height_stddev_, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyEpaQuantiles) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            expected_, expected_trend_, trend_steps_,
            energy_equation_, nutrient_ranges_, nutrient_equations_,
            food_prices_, weight_quantiles_, empty_epa_quantiles,
            height_stddev_, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyHeightStdDev) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            ...
            energy_equation_, nutrient_ranges_, nutrient_equations_,
            food_prices_, weight_quantiles_, epa_quantiles_,
            empty_height_stddev, height_slope_),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ConstructorWithEmptyHeightSlope) {
    ...
    EXPECT_THROW(
        KevinHallModel(
            ...
            energy_equation_, nutrient_ranges_, nutrient_equations_,
            food_prices_, weight_quantiles_, epa_quantiles_,
            height_stddev_, empty_height_slope),
        HgpsException);
}
...
TEST_F(KevinHallModelTest, ComputeEEMethod) {
    ...
    // Expected: (K + gamma_F * F + gamma_L * L + delta * BW + beta_TEF + beta_AT + x * (EI - rho_G * 0.0)) / (1 + x)
    EXPECT_TRUE(std::isfinite(result));
}
...
TEST_F(KevinHallModelTest, GetExpectedMethod) {
    ...
    auto mock_context = std::make_shared<RuntimeContext>(
        nullptr, nullptr, nullptr);
    
    // Test getting expected nutrient intake
    double result = model.get_expected(*mock_context, Gender::male, 30, "Carbohydrate"_id, std::nullopt, false);
...
TEST_F(KevinHallModelTest, GetExpectedEnergyIntake) {
    ...
    double result = model.get_expected(*mock_context, Gender::male, 30, "EnergyIntake"_id, std::nullopt, false);
...
TEST_F(KevinHallModelTest, GetExpectedWeight) {
    ...
    double result = model.get_expected(*mock_context, Gender::male, 30, "Weight"_id, std::nullopt, false);
...
TEST_F(KevinHallModelTest, GetExpectedHeight) {
    ...
    double result = model.get_expected(*mock_context, Gender::male, 30, "Height"_id, std::nullopt, false);
...
TEST_F(KevinHallModelTest, GetExpectedPhysicalActivity) {
    ...
    double result = model.get_expected(*mock_context, Gender::male, 30, "PhysicalActivity"_id, std::nullopt, false);
...
TEST_F(KevinHallModelTest, GetExpectedUnknownFactor) {
    ...
    double result = model.get_expected(*mock_context, Gender::male, 30, "UnknownFactor"_id, std::nullopt, false);
