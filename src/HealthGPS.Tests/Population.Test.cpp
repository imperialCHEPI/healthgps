...
TEST(TestPerson, CopyOperations) {
    ...
    ASSERT_EQ(source.risk_factors["PhysicalActivity"_id], destination.risk_factors["PhysicalActivity"_id]);
}
...
TEST(TestStaticLinearModel, BasicOperations) {
    ...
    auto definition = StaticLinearModelDefinition(
        ...
        std::vector<core::DoubleInterval>{},
        Eigen::MatrixXd(0, 0),
...
TEST(TestColumnNumeric, DataAccessAndNulls) {
    ...
    auto col = std::make_unique<DoubleDataTableColumn>("test_col", std::move(data), std::move(bitmap));
...
TEST(TestColumnNumeric, Cloning) {
    ...
    std::vector<double> data{1.0, 2.0, 3.0};
    std::vector<bool> bitmap{true, false, true};
    auto original = std::make_unique<DoubleDataTableColumn>("test_col", std::move(data), std::move(bitmap));
...
TEST(TestColumnPrimitive, ComprehensiveVisitorImplementation) {
    ...
    auto str_col = std::make_unique<StringDataTableColumn>("string_col", std::move(str_data), std::move(str_bitmap));
    
    std::vector<float> float_data{1.5f, 2.5f};
    std::vector<bool> float_bitmap{false, true};
    auto float_col = std::make_unique<FloatDataTableColumn>("float_col", std::move(float_data), std::move(float_bitmap));
    
    std::vector<double> double_data{1.5, 2.5};
    std::vector<bool> double_bitmap{true, true};
    auto double_col = std::make_unique<DoubleDataTableColumn>("double_col", std::move(double_data), std::move(double_bitmap));
    
    std::vector<int> int_data{1, 2};
    std::vector<bool> int_bitmap{true, false};
    auto int_col = std::make_unique<IntegerDataTableColumn>("int_col", std::move(int_data), std::move(int_bitmap));
...
TEST(TestDataTable, DemographicOperations) {
    ...
    auto region_prob_col = std::make_unique<DoubleDataTableColumn>("region_prob", std::move(region_prob_data));
...
TEST(TestDataTable, ComprehensiveOperations) {
    ...
    auto ethnicity_dist = demo_table.get_ethnicity_distribution(25, Gender::female, Region::England);
