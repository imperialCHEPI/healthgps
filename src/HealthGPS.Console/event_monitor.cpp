...
void EventMonitor::visit(const hgps::ResultEventMessage &message) {
    ...
    try {
        ...
        std::cout << "DEBUG: [EventMonitor] Result data: population=" << message.content.population_size 
        ...
        std::cout << "DEBUG: [EventMonitor] Series data: " 
                  << message.content.series.size() << " channels, "
                  << message.content.series.sample_size() << " samples" << std::endl;
                  
        if (message.content.series.channels().empty()) {
            std::cerr << "ERROR: [EventMonitor] Result message has NO CHANNELS. Output will be incomplete!" << std::endl;
        } else {
        ...
        std::cerr << "ERROR: [EventMonitor] Exception while processing result message: " << e.what() << std::endl;
        ...
        try {
            ...
            std::cerr << "RECOVERY FAILED: Could not write result data: " << recovery_e.what() << std::endl;
        }
    } catch (...) {
        std::cerr << "ERROR: [EventMonitor] Unknown exception while processing result message" << std::endl;
    }
}
