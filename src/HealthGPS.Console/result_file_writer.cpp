...
ResultFileWriter::~ResultFileWriter() {
    ...
    if (stream_.is_open()) {
        ...
        try {
            ...
            std::cerr << "ERROR: [ResultFileWriter] Exception while closing JSON file: " << e.what()
                      << std::endl;
        }
    }
    ...
    if (csvstream_.is_open()) {
        ...
        try {
            ...
            std::cerr << "ERROR: [ResultFileWriter] Exception while closing CSV file: " << e.what()
                      << std::endl;
        }
    }
}
...
void ResultFileWriter::write(const hgps::ResultEventMessage &message) {
    ...
    try {
        ...
        if (!stream_.is_open() || !stream_.good()) {
            std::cerr << "ERROR: [ResultFileWriter] JSON output stream is not open or in bad state"
                      << std::endl;
            return;
        }

        if (!csvstream_.is_open() || !csvstream_.good()) {
            std::cerr << "ERROR: [ResultFileWriter] CSV output stream is not open or in bad state"
                      << std::endl;
            return;
        }
        ...
        std::cerr << "ERROR: [ResultFileWriter] Unknown exception while writing results"
                  << std::endl;
    }
}
