...
**Diagrams:** SVG figures live under `documentation/images/`. **Mermaid** (````mermaid`blocks) renders on the **built GitHub Pages site** via`[_includes/head-custom.html](_includes/head-custom.html)`. On **github.com** when viewing` .md` source, Mermaid often appears as code, hence use the deployed site to review diagrams.
