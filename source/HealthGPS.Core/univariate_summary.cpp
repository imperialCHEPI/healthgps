...
namespace hgps {
	namespace core {
		...
		UnivariateSummary::UnivariateSummary(const std::vector<double> values)
			:UnivariateSummary() {
			append(values);
		}
		...
		unsigned int UnivariateSummary::count_null() const noexcept {
			return null_count_;
		}
		...
		double UnivariateSummary::sum() const noexcept {
			return moments_[0] * moments_[1];
		}

		double UnivariateSummary::average() const noexcept {
			return moments_[1];
		}
		...
		double UnivariateSummary::std_deviation() const noexcept {
			return std::sqrt(variance());
		}

		double UnivariateSummary::std_error() const noexcept{
			return std::sqrt(variance() / moments_[0]);
		}
		...
		double UnivariateSummary::kurtosis() const noexcept	{
			...
			double v = variance();
			return (moments_[4] * moments_[0] * moments_[0] * 
				   (moments_[0] + 1.0) / (v * v * n1) - n1 * n1 * 3.0) / k_fact;
		}
		...
		void UnivariateSummary::append(const double value) noexcept {
			if (is_empty())	{
				min_ = value;
				max_ = value;
			}
		...
		void UnivariateSummary::append_null() noexcept {
			null_count_++;
		}

		void UnivariateSummary::append_null(const unsigned int length) noexcept {
			null_count_+= length;
		}
