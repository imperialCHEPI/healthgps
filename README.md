...
stylistic problems with C++ code.

To run `clang-tidy` with the default checks we have enabled for this repository, please
invoke the included Python script, passing in the path to the build folder, e.g.:

```cmd
> python ./run-clang-tidy.py -p out/build/linux-debug
```
